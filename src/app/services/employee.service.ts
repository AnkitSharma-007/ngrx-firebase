...
export class EmployeeService {
  ...
  getCityList(): Observable<City[]> {
    const cityList = this.db.collection<City>('City').snapshotChanges().pipe(
      map(actions => {
        return actions.map(
          c => ({
  ...
  getAllEmployees(): Observable<Employee[]> {
    const employeeList = this.db.collection<Employee>('Employee').snapshotChanges().pipe(
      map(actions => {
        return actions.map(
          c => ({
  ...
  getEmployeeById(employeeId: string): Observable<Employee> {
    const employeeData = this.db.doc<Employee>('Employee/' + employeeId).valueChanges();
    return employeeData;
  }
